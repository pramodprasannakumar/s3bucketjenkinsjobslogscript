...
S3_BUCKET="s3://backupfromjenkins"  # Replace with your S3 bucket name
S3_FOLDER="jenkinsjobslog"
...
for job_dir in "$JENKINS_HOME/jobs/"*/; do
    ...
    for build_dir in "$job_dir/builds/"*/; do
        ...
        if [ -f "$log_file" ] && [ "$(date -r "$log_file" +%Y-%m-%d)" == "$DATE" ]; then
            ...
            aws s3 cp "$log_file" "$S3_BUCKET/$S3_FOLDER/$job_name-$build_number.log" --only-show-errors
            
            if [ $? -eq 0 ]; then
                echo "Uploaded: $job_name/$build_number to $S3_BUCKET/$S3_FOLDER/$job_name-$build_number.log"
            else
